...
*Setup instructions for a gitian build of Fastcoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fastcoin
...
- [Building Fastcoin](#building-fastcoin)
...
Clone the git repositories for fastcoin and gitian and then checkout the fastcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the fastcoin repository
...
To build Fastcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the fastcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fastcoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/fastcoin-project/fastcoin
...
For example:
```bash
URL=https://github.com/user/fastcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit fastcoin=${COMMIT} --url fastcoin=${URL} ../fastcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fastcoin=${COMMIT} --url fastcoin=${URL} ../fastcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fastcoin=${COMMIT} --url fastcoin=${URL} ../fastcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fastcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fastcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/fastcoin-build.assert
```
